...
export function easternNextCalendarDayIso(fromInstant: Date): string {
  const startKey = formatEasternCalendarDateKey(fromInstant);
  let t = fromInstant.getTime();
  for (let i = 0; i < 72; i += 1) {
    t += 60 * 60 * 1000;
    const key = formatEasternCalendarDateKey(new Date(t));
    if (key !== startKey) {
      return key;
    }
  }
  return formatEasternCalendarDateKey(
    new Date(fromInstant.getTime() + 48 * 60 * 60 * 1000)
  );
}
...
export function siteAlertsNewCatalogDefaults(now = new Date()): CatalogRow {
  const startDateIso = formatEasternCalendarDateKey(now);
