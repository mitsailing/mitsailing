...
describe('usersAdminHandlers', () => {
  it('admin lists users with boolean ban state', async () => {
  ...
  it('admin gets one user or null', async () => {
  ...
  it('admin creates users and maps create failures', async () => {
    await expect(
  ...
  it('admin updates details, ban state, and password', async () => {
    mocks.userFindUnique.mockResolvedValue({
      banned: false,
      role: Role.USER,
    });
  ...
  it('admin unbans users and reports no data updates', async () => {
    mocks.userFindUnique.mockResolvedValue({
      banned: true,
      role: Role.USER,
    });
  ...
  it('admin blocks last-admin demotion or ban', async () => {
    mocks.userFindUnique.mockResolvedValue({
      banned: false,
      role: Role.ADMIN,
    });
  ...
  it('admin demotes an admin when another admin remains', async () => {
    mocks.userFindUnique.mockResolvedValue({
      banned: false,
      role: Role.ADMIN,
    });
  ...
  it('admin sees validation and missing-user errors on update', async () => {
    await expect(
      usersAdminHandlers.updateFromForm(
        'user-1',
  ...
  it('admin deletes users with guard rails', async () => {
  ...
  it('admin create failure maps missing user ids to unknown', async () => {
  ...
  it('admin update surfaces network failures from auth APIs', async () => {
    mocks.userFindUnique.mockResolvedValue({
      banned: false,
      role: Role.USER,
    });
