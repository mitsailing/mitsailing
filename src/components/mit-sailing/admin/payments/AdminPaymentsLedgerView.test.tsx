...
describe('AdminPaymentsLedgerView', () => {
  ...
  it('renders empty state when no payments match filters', () => {
    ...
    expect(
      screen.getByText('No payments match those filters.')
    ).toBeVisible();
  });
  ...
  it('links event payment title to the event page', () => {
    ...
    expect(
      screen.getByRole('link', { name: 'Spring Series' })
    ).toHaveAttribute('href', '/events/spring-series');
  });
  ...
  it('shows latest webhook event type and date in the health summary', () => {
    ...
    expect(
      screen.getByText(/payment_intent\.succeeded/)
    ).toBeVisible();
  });
});
