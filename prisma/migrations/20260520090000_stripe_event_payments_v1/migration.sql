...
BEGIN
  IF EXISTS (
    ...
    GROUP BY "id", "event_id", "user_id"
    HAVING count(*) > 1
  ) THEN
    RAISE EXCEPTION
      'event_registrations contains duplicate (id, event_id, user_id) rows; remediate rows before creating event_registrations_id_event_id_user_id_key';
  END IF;
END
$$;
