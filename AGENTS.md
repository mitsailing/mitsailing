...
- Treat commands, repository state, GitHub state, CI status, deployment state, package availability, docs, and current tool behavior as facts to verify from the source of truth before claiming them.
- Separate verified facts from assumptions. If a statement depends on memory, convention, or inferred behavior, say that and verify before acting on it.
- Before high-impact actions such as merge, force-push, branch deletion, branch protection edits, production data changes, migrations, payments, auth, or deployment changes, run a preflight that checks current state, allowed actions, expected visible result, and rollback or recovery path.
- After high-impact actions, verify the actual remote or runtime result before saying it is done. For GitHub merges, query the PR and commit state and confirm the commit title/link that will be visible on `main`.
- When the user corrects an agent mistake that reveals a repeatable workflow gap, stop defending the prior answer, identify the failed assumption, update the relevant rule/runbook/skill, and continue from the corrected source of truth.
- If a requested repair would require rewriting protected history, changing branch protection, deleting data, or otherwise bypassing safeguards, report the verified blocker and do not perform the bypass unless the user explicitly authorizes that specific action.
